...
while fairly simple, my goal was to create a page that uses *pure css*. there
are only two HTML tags used--the opening and closing `<style>` and `</style>`
tags required to add css to the page. however, all modern browsers will create
an `<html>`, `<head>`, and `<body>` tag when those are missing from the page.
this allows me, in spite of not explicitly including any elements on my page,
to make use of 4 css pseudo-elements: `html::before`, `html::after`,
`body::before`, and `body::after`. i use this to add 2 hexagons and 2 words to
the page.
